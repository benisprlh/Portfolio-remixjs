...
const Navbar = () => {
    ...
    return (
        <nav className="py-10 flex justify-between items-center z-50 lg:pb-10 pb-20">
            <h1 className="text-2xl font-bold underline underline-offset-8 decoration-green-500 -rotate-2 text-gray-200">
                Beni Saprulah 👨🏻‍💻
            </h1>
                ...
                {socials.map((social, index) => {
                    ...
                    return (
                        <a href={social.link} key={index} aria-label={social.label} target="_blank" rel="noopener noreferrer"> {/* Use anchor tag for external links */}
                            <Icon className="w-6 h-6 hover:scale-125 transition-all hover:cursor-pointer text-gray-200" />
                        </a>
                    );
                })}
            </div>
